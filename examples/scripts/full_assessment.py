...
def generate_summary_report(assessment_dir, all_results):
    ...
    if 'serverdrift' in all_results:
        for test_name, test_results in all_results['serverdrift'].get('tests', {}).items():
            for result in test_results.get('results', []):
                if isinstance(result, dict) and (not result.get('status_match', True) or not result.get('cookies_match', True)):
                    findings.append({
                        'module': 'ServerDrift',
                        'test': test_name,
...
def main():
    ...
    try:
        ...
        all_results['bypassgen'] = run_bypassgen(
            args.target, assessment_dir, args.auth_file, args.rate_limit, args.verify_exploits, args.verbose
        )
