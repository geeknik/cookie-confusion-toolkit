...
def compare_header_injection(clientfork, browsers):
    ...
    return {
        "standard_crlf": results,
        "advanced_crlf": advanced_results
    }
...
def compare_cookie_shadowing(clientfork, browsers):
    ...
    standard_results = clientfork.test_cookie_shadowing(
        cookie_name="session",
        browsers=browsers
    )
    ...
    domain_variations = [
        {"cookie_name": "domain_cookie", "variations": [
            {"name": "domain_cookie", "value": "original", "domain": "example.com", "secure": True, "httpOnly": True},
    ...
    path_variations = [
        {"cookie_name": "path_cookie", "variations": [
            {"name": "path_cookie", "value": "original", "path": "/admin", "secure": True, "httpOnly": True},
...
def generate_report(comparison_results, differences, output_dir, target):
    ...
    with open(text_report_file, 'w') as f:
        ...
        if differences:
            ...
            f.write(textwrap.fill(
                "Key security concerns from these differences:",
                width=80
            ) + "\n\n")
