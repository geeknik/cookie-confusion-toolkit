...
from .utils.cookie_utils import Cookie, create_malformed_cookie, detect_cookie_parser
...
class ServerDrift:
    ...
    def test_key_overwrite(
        self, cookie_name: str = "session", variations: List[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        test_formats = [
            # Multiple cookies in one header
            {
                ...
                "header": lambda values: {
                    "Cookie": "; ".join([f"{cookie_name}={v['value']}" for v in values])
                },
            },
            # Multiple separate cookie headers
            {
                ...
                "header": lambda values: {
                    f"Cookie{i}": f"{cookie_name}={v['value']}" for i, v in enumerate(values)
                },
            },
            ...
            # Multiple cookies with different casing
            {
                ...
                "header": lambda values: {
                    "Cookie": f"{cookie_name.lower()}={values[0]['value']}; {cookie_name.upper()}={values[1]['value']}; {cookie_name}={values[2]['value']}"
                },
            },
        ]
    ...
    def test_attribute_truncation(
        self, cookie_name: str = "session", attributes: List[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        for attr in attributes:
            ...
            rate_limit(self.rate_limit_delay)
            full_response = safe_request(self.target, headers={"Cookie": full_cookie})

            # Test with truncated attribute
            rate_limit(self.rate_limit_delay)
            truncated_response = safe_request(self.target, headers={"Cookie": truncated_cookie})
            ...
            if self.verbose:
                logger.debug(
                    f"Attribute {attr_name}: Full={full_response.status_code}, Truncated={truncated_response.status_code}"
                )

        self.results["tests"]["attribute_truncation"] = results
        return results

    def test_samesite_domain_logic(
        self,
        cookie_name: str = "session",
        domain_variations: List[str] = None,
        samesite_values: List[str] = None,
    ) -> Dict[str, Any]:
        """
        ...
        for domain in domain_variations:
            ...
            result["accepted"] = cookie_name in cookies_set or any(
                cookie_name in h for h in set_cookie_headers
            )
            ...
            if self.verbose:
                logger.debug(
                    f"Domain {domain}: {response.status_code}, Accepted: {result['accepted']}"
                )
        ...
        for samesite in samesite_values:
            ...
            if self.verbose:
                logger.debug(
                    f"SameSite {samesite}: {response.status_code}, Accepted: {result['accepted']}"
                )
    ...
    def test_malformed_cookies(self, malformations: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        if malformations is None:
            malformations = [
                ...
                {
                    "type": "attribute_without_semicolon",
                    "name": "session",
                    "value": "value",
                    "attribute": "Path=/",
                },
        ...
        for test in malformations:
            ...
            errors_found = [
                indicator for indicator in error_indicators if indicator in response_body[:1000]
            ]
            ...
            if self.verbose:
                logger.debug(
                    f"Malformation {malformation_type}: {cookie_str} -> {response.status_code}"
                )
    ...
    def analyze_framework_specific(self) -> Dict[str, Any]:
        ...
        if framework in ["express", "nodejs"]:
            ...
            rate_limit(self.rate_limit_delay)
            response = safe_request(self.target, headers={"Cookie": json_cookie})
        ...
        # Flask/Python specific tests
            ...
            rate_limit(self.rate_limit_delay)
            response = safe_request(self.target, headers={"Cookie": flask_cookie})
            ...
            rate_limit(self.rate_limit_delay)
            response = safe_request(self.target, headers={"Cookie": django_cookie})
        ...
        # Java/Spring specific tests
            ...
            rate_limit(self.rate_limit_delay)
            response = safe_request(self.target, headers={"Cookie": spring_cookie})
            ...
            spring_cookies = ["JSESSIONID=uppercase", "jsessionid=lowercase"]

            case_results = {}
            for cookie in spring_cookies:
                rate_limit(self.rate_limit_delay)
                response = safe_request(self.target, headers={"Cookie": cookie})
        ...
        # PHP specific tests
            ...
            rate_limit(self.rate_limit_delay)
            response = safe_request(self.target, headers={"Cookie": php_cookie})
            ...
            rate_limit(self.rate_limit_delay)
            response = safe_request(self.target, headers={"Cookie": php_serialized})
