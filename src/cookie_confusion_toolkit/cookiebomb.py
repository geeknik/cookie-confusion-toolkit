...
class CookieBomb:
    ...
    def test_key_collisions(
        self, cookie_names: List[str], variations: Optional[List[Dict[str, Any]]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if variations is None:
            variations = [
                {"name": name, "value": f"value_{i}"} for i, name in enumerate(cookie_names)
            ]
        ...
        for i, variant in enumerate(variations):
            cookie_str = create_cookie_collision(
                cookie_names[0] if cookie_names else "session", [variant]
            )[0]
    ...
    def test_overlong_values(
        self, cookie_name: str = "session", lengths: List[int] = [100, 1000, 4000, 8000]
    ) -> Dict[str, Any]:
        """
        ...
        for length in lengths:
            ...
            if self.verbose:
                logger.debug(
                    f"Length {length}: {cookie_name}=[{length} chars] -> {response.status_code}"
                )
    ...
    def test_path_scoping(
        self, cookie_name: str = "session", paths: List[str] = ["/", "/admin", "/api", "/%61dmin"]
    ) -> Dict[str, Any]:
        """
        ...
        for i, path in enumerate(paths):
            ...
            cookie = Cookie(name=cookie_name, value=value, path=path)

            # Set the cookie
            rate_limit(self.rate_limit_delay)
            set_response = safe_request(
                self.target, method="GET", headers={"Set-Cookie": cookie.to_set_cookie_header()}
            )
    ...
    def run_all_tests(
        self, cookie_names: List[str] = ["session", "sessionid", "SESSIONID"]
    ) -> Dict[str, Any]:
        """
    ...
    def generate_custom_test(self, test_type: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dict[str, Any]: Test results
        """
        results = {"description": f"Custom test: {test_type}", "params": params, "results": []}
        ...
        if test_type == "malformed_cookie":
            ...
            cookie_str = create_malformed_cookie(cookie_name, value, malformation_type, **params)
            ...
            rate_limit(self.rate_limit_delay)
            response = safe_request(self.target, headers={"Cookie": cookie_header})
