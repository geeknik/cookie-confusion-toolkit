...
def is_valid_target(url: str) -> bool:
    ...
    restricted_domains = [
        "gov", "mil", "edu", "bank", "healthcare", "nhs.uk", 
        "irs.gov", "cisa.gov", "fbi.gov", "police"
    ]
...
def ethical_check(target: str) -> bool:
    ...
    sensitive_domains = [
        # Government
        ".gov", ".mil", ".gc.ca", ".gov.uk", ".gouv.fr", ".gov.au", 
        # Critical infrastructure
        "energy", "power", "water", "utility", "nuclear", "electric",
        # Financial
        "bank", "credit", "loan", "finance", "insurance", "investment",
        # Healthcare
        "health", "hospital", "clinic", "medical", "patient", "pharma",
        # Education
        ".edu", ".ac.uk", ".edu.au", "school", "university", "college",
    ]
    
    for domain in sensitive_domains:
        if domain in hostname:
            logger.error(f"Testing on {hostname} requires explicit permission as it may be a sensitive domain")
